...
namespace TS_ENGINE
{
	...
	{
		nlohmann::json::value_type SceneSerializer::SerializeNode(nlohmann::json::value_type jNode, Node* node)
		{
			jNode[node->GetName()] = {
				{"Name", node->GetName()},
				{"EntityID", node->GetEntityID()},
				{"HasGameObject", node->HasGameObject()}
			};

			if (node->HasGameObject())//GameObject
			{
				jNode[node->GetName()]["GameObject"];
				jNode[node->GetName()]["GameObject"]["Name"] = node->GetGameObject()->GetName();
				jNode[node->GetName()]["GameObject"]["PrimitiveType"] = node->GetGameObject()->GetPrimitiveType();
				jNode[node->GetName()]["GameObject"]["IsRendererActive"] = node->GetGameObject()->IsMeshRendererActive;
				
				jNode[node->GetName()]["GameObject"]["MeshRenderer"]["MaterialProperties"] =
				{					
					{"Name", node->GetGameObject()->GetMeshRenderer()->GetMaterial()->GetMaterialProperties()->GetName()},
		...
		void SceneSerializer::Save(TS_ENGINE::SceneManager::Scene& scene)
		{
			nlohmann::json json;
			...
			json = SerializeNode(json["Scene"], scene.GetNode());

			std::string filePath = "D:\\Documents\\ThinkSideways\\ThinkSideways\\build\\x64\\Debug\\bin\\SavedScenes\\" + std::string(sceneName) + ".json";
		...
		std::string SceneSerializer::ReadSceneFromJson(std::string path)
		{
			...
			Logger::Print("[Info] ReadSceneFromJson!");
			return finalText;
		}
		...
		Node* SceneSerializer::DeserializeNode(nlohmann::json jsonNode)
		{
			...
			TS_ENGINE::PrimitiveType primitiveType;
			...
			TS_ENGINE::SceneManager::Node* node = NULL;
		...
		void SceneSerializer::Load(const nlohmann::json& j, TS_ENGINE::SceneManager::Scene& scene)
		{
			...
			Logger::Print("[Info]Deserialized Scene!");
		}
	}
}
